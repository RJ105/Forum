...
function getlikes($threadid)
 {
  global $conn;
    ...
    $sql = "SELECT `thread_id`, `thread_title`,`thread_user_id`,DATE_FORMAT(timestamp,'%d-%m-%y') AS timestamp FROM `Threads` where `thread_id` NOT IN (SELECT DISTINCT `comment_thread_id` FROM `Comments`) ORDER BY `thread_id`";
